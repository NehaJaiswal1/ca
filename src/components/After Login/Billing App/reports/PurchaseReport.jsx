...
import {
    getPurchaseReport
} from "../../../../Redux/PurchaseReport/PurchaseReport.Action";
...
// const firmId = '652d0f5d4115c16957111ed4';
    ...
    const handleDateOptionChange = (e) => {
        ...
        if (selectedOption === 'default') {
            ...
            // For "Custom" option, do not set "filteredPurchaseData" here, just update the "fromDate" and "toDate" values
            ...
            setFilteredPurchaseData(filterDataByCustomDate(dateOptions[selectedOption].from, dateOptions[selectedOption].to));
    ...
    const handleFromDateChange = (e) => {
        ...
        if (selectedDateOption === 'Custom') {
            setFilteredPurchaseData(filterDataByCustomDate(e.target.value, toDate));
        }
    };
    ...
    const handleToDateChange = (e) => {
        ...
        if (selectedDateOption === 'Custom') {
            setFilteredPurchaseData(filterDataByCustomDate(fromDate, e.target.value));
        }
    };

    const filterDataByCustomDate = (startDate, endDate) => {
        const newData = PurchaseData?.filter((item) => {
    ...
    const handleSearch = (e) => {
        ...
        if (query === '') {
            ...
            setFilteredPurchaseData(PurchaseData?.filter((data) =>
    ...
    const totalTransactions = filteredPurchaseData?.length || 0;
    const totalSales = filteredPurchaseData?.reduce((acc, data) => acc + (data.finalAmount || 0), 0) || 0;
    const totalDue = filteredPurchaseData?.reduce((acc, data) => acc + (data.dueAmount || 0), 0) || 0;
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    >
                        <option value="default">All Purchases</option>
